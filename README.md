...
## 시간대(cron) 튜닝
`.github/workflows/menu.yml` 의 `schedule` 은 **KST 09:00~12:59** 를 10분 간격으로 폴링한다.
추가로 `check_menu.py` 의 `SEND_AFTER_HOUR_KST`(기본 9) 가드로 **09시 이전에는 전송하지 않는다**
(9시 이전에 메뉴가 감지돼도 상태를 저장하지 않아, 9시 이후 첫 폴링이 전송).
- 전송 시각을 바꾸려면: cron 시작 시각 + `SEND_AFTER_HOUR_KST`(workflow env) 를 함께 조정.
