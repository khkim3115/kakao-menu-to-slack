...
둘 다 같은 워크플로를 깨우며, 중복 방지(`last_key`/`shortcode` 비교 + 하루 1건 캡 + `concurrency`)
덕분에 메시지는 하루 1건만 간다.

**메뉴 소스마다 잡이 하나씩**이다(워크플로가 둘이라 dispatch URL 도 둘).

```
cron-job.org "kakao cron" ───────(KST 평일 09:05~12:50, 15분)─┐
                                                              ├─> menu.yml         → check_menu.py
GitHub schedule ─────────────────(KST 평일 07:04~12:54, 10분)─┘

cron-job.org "byeolsikdang cron" ─(KST 평일 09:05~11:50, 15분)─┐
                                                               ├─> byeolsikdang.yml → check_byeolsikdang.py
GitHub schedule ─────────────────(KST 평일 07:09~14:54, 15분)──┘
```

### cron-job.org 잡 설정값

두 잡은 **URL 과 스케줄만 다르고** 나머지(Method·Body·헤더·PAT·타임존)는 동일하다.

| 항목 | kakao cron | byeolsikdang cron |
|---|---|---|
| URL 의 워크플로 파일명 | `.../actions/workflows/`**`menu.yml`**`/dispatches` | `.../actions/workflows/`**`byeolsikdang.yml`**`/dispatches` |
| 스케줄 — 시 | `9-12` (16회/일) | `9-11` (12회/일) |
| 스케줄 — 분 | `5,20,35,50` | `5,20,35,50` |
| 스케줄 — 요일 | 월~금 | 월~금 |

공통값 — URL 앞부분 `https://api.github.com/repos/khkim3115/kakao-menu-to-slack` ·
Method `POST` · Body `{"ref":"main"}` · 헤더 `Authorization: Bearer <PAT>` /
`Accept: application/vnd.github+json` / `Content-Type: application/json` ·
타임존 `Asia/Seoul` · 실행 실패 시 이메일 알림 ON.

> **`inputs` 없이 dispatch 하면** 워크플로의 `test` 기본값 `false` 가 적용돼 **운영 경로**(공유 채널)로
> 동작한다. test 모드(나와의 대화)는 Actions UI 에서 수동으로 `test=true` 를 줄 때만 켜진다.

별식당 잡의 시(hour) 범위가 더 좁은 건 의도다 — 메뉴판을 찾기 전까진 폴링마다 **유료 Apify 호출**이
발생하므로(찾은 뒤엔 하루 1건 캡이 이후 폴링을 전부 무료로 단락), 메뉴가 안 올라온 날의 최대 비용을
12회로 묶는다. 관측상 메뉴판은 08:5x~09:16 에 올라와 09:05 첫 핑이면 충분하다.

요일을 월~금으로 좁혀두면 주말 실행 이력이 쌓이지 않는다. 설령 주말·공휴일에 핑이 가더라도
스크립트의 주말·공휴일 가드가 (별식당은 유료 Apify 호출 **전에**) 즉시 종료시키므로 비용·오발송은 없다
— 공휴일은 cron 으로 표현할 수 없어 이 가드가 최종 방어선이다.
...
2. **GitHub Actions 실행 이력** — `gh run list --workflow=menu.yml`(별식당은 `--workflow=byeolsikdang.yml`)
   에 오늘 run 이 있나? **런이 아예 없으면** 스크립트가 아니라 트리거 문제다(1번으로).
   즉시 복구는 `gh workflow run <워크플로>.yml --ref main` — 중복 방지 가드가 있어 이중 전송은 없다.
...
### 동작 원리
```
트리거 이중화 — cron-job.org(주, KST 평일 09:05~11:50) + GitHub cron(백업, KST 평일 07:09~14:54)
...
4. **cron-job.org 잡 등록**(필수): GitHub cron 단독으로는 발화가 안 되는 날이 있다 —
   위 [트리거 이중화](#트리거-이중화-외부-크론--github-cron) 의 `byeolsikdang cron` 설정값 참고.
   기존 카카오 잡을 Clone 해 **URL 의 워크플로 파일명과 시(hour) 범위만** 바꾸면 된다(PAT 재사용).
   > 검증 요령: **이미 그날 전송이 끝난 뒤** cron-job.org 의 `TEST RUN` 을 누르면, 하루 1건 캡이
   > `[skip] 오늘 이미 전송함` 으로 단락시켜 **채널 중복 게시도 Apify 과금도 없이** 트리거 배선만
   > 확인할 수 있다. 로그에 `TEST_MODE: 0` 이 찍히면 외부 트리거가 운영 경로로 도달한 것이다.
